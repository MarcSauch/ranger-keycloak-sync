...
class SyncRunner:
    ...
    @staticmethod
    def _normalize_filter_values(values: Set[str] | List[str]) -> Set[str]:
        return {value.strip() for value in values if value and value.strip()}

    def _resolve_filter(
        self,
        default_values: Set[str],
        override_values: Set[str] | None,
        merge_with_defaults: bool,
    ) -> Set[str]:
        if override_values is None:
            return set(default_values)

        normalized_override = self._normalize_filter_values(override_values)
        if merge_with_defaults:
            return set(default_values) | normalized_override
    ...
    def _build_mapping(
        ...
        effective_users = self._resolve_filter(self.include_users, include_users, merge_with_defaults)
        effective_roles = self._resolve_filter(self.include_roles, include_roles, merge_with_defaults)
        effective_groups = self._resolve_filter(self.include_groups, include_groups, merge_with_defaults)
        ...
        for user in users:
            ...
            if not username or not user_id:
                continue
            if effective_users and username not in effective_users:
                continue
            ...
            for role in roles:
                if effective_roles and role not in effective_roles:
                    continue
                role_to_users[role].add(ranger_username)

            groups = self.keycloak.user_groups(user_id)
            for group in groups:
                if effective_groups and group not in effective_groups:
                    continue
        ...
        filter_summary = {
            "users": sorted(effective_users),
            "roles": sorted(effective_roles),
            "groups": sorted(effective_groups),
        }
        return role_to_users, group_to_users, filter_summary
    ...
    def run_sync_once(
        ...
        role_mapping, group_mapping, filter_summary = self._build_mapping(
        ...
        total_users = set()
        for members in role_mapping.values():
            total_users.update(members)
        for members in group_mapping.values():
            total_users.update(members)

        return {
            "synced_users": len(total_users),
            "synced_roles": len(role_mapping),
            "synced_groups": len(group_mapping),
    ...
    def run_forever(self) -> None:
        while True:
            try:
                summary = self.run_sync_once()
                LOG.info("Sync completed")
                LOG.info(
                    "Sync summary users=%d roles=%d groups=%d",
                    summary["synced_users"],
                    summary["synced_roles"],
                    summary["synced_groups"],
                )
            except Exception as exc:
